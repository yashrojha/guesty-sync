					...
					<div class="guesty-single-property-wrapper">
						...
						<div class="property-main-content elementor-container">
							<div class="content-left">
								...
								// Count only BEDROOM-type rooms; fall back to the synced guesty_bedrooms value
								$_custom_brs_specs    = get_post_meta($post_id, 'guesty_custom_bedrooms', true);
								$_bedroom_count_specs = get_post_meta($post_id, 'guesty_bedrooms', true);
								if (is_array($_custom_brs_specs) && !empty($_custom_brs_specs)) {
									$_bedroom_count_specs = count(array_filter($_custom_brs_specs, function($r) {
										return in_array($r['type'] ?? 'BEDROOM', ['BEDROOM', ''], true);
									}));
								}
								if ($_bedroom_count_specs) $specs_array[] = $_bedroom_count_specs . ' BEDROOMS';
								...
								<div class="guesty-accordions">
									...
									endforeach; ?>
									<?php
							// Build bedroom slides from the custom-managed bedrooms meta
							$custom_bedrooms = get_post_meta($post_id, 'guesty_custom_bedrooms', true);
							if (!is_array($custom_bedrooms)) $custom_bedrooms = [];

							// Derive summary counts — BEDROOM and LIVING_ROOM types both contribute
							$_br_total   = 0;
							$_beds_total = 0;
							foreach ($custom_bedrooms as $_cbr) {
								$_type = $_cbr['type'] ?? 'BEDROOM';
								if (!in_array($_type, ['BEDROOM', 'LIVING_ROOM', ''], true)) continue;
								if (in_array($_type, ['BEDROOM', ''], true)) $_br_total++;
								foreach (($_cbr['beds'] ?? []) as $_bed) {
									$_beds_total += (int) ($_bed['quantity'] ?? 0);
								}
							}
							$rooms_array = [];
							if ($_br_total)   $rooms_array[] = $_br_total . ' BEDROOMS';
							if ($_beds_total) $rooms_array[] = $_beds_total . ' BEDS';
							$combined_rooms = implode(' • ', $rooms_array);

							// Show BEDROOM and LIVING_ROOM rooms that have a photo
							// Bedrooms also require at least one bed; living rooms show with just a photo
							$bedroom_slides = [];
							$bedroom_seq    = 0;
							$living_seq     = 0;
							foreach ($custom_bedrooms as $index => $bedroom) {
								$room_type = $bedroom['type'] ?? 'BEDROOM';

								// Skip bathroom types
								if (!in_array($room_type, ['BEDROOM', 'LIVING_ROOM', ''], true)) continue;

								$is_living_room = ($room_type === 'LIVING_ROOM');
								if (!$is_living_room) $bedroom_seq++;
								else $living_seq++;

								$image_id    = $bedroom['image_id'] ?? '';
								$bed_details = [];
								foreach (($bedroom['beds'] ?? []) as $bed) {
									$count = (int) ($bed['quantity'] ?? 1);
									$type  = str_replace('_', ' ', $bed['type'] ?? '');
									if ($count > 1) $type .= 'S';
									$bed_details[] = $count . ' ' . $type;
								}
								$bed_display_text = implode(', ', array_filter($bed_details));

								// Bedrooms need a photo + bed info; living rooms only need a photo
								if (empty($image_id)) continue;
								if (!$is_living_room && empty($bed_display_text)) continue;

								if (!empty($bedroom['name'])) {
									$room_title = $bedroom['name'];
								} elseif ($is_living_room) {
									$room_title = $living_seq > 1 ? 'Living Room ' . $living_seq : 'Living Room';
								} else {
									$room_title = 'Bedroom ' . $bedroom_seq;
								}
												...
												<div class="swiper-wrapper">
													<?php foreach ($bedroom_slides as $slide) : ?>
														<div class="swiper-slide">
															...
															<h4 class="room-title"><?php echo esc_html($slide['room_title']); ?></h4>
															<?php if (!empty($slide['bed_display_text'])) : ?>
							...
							<aside class="content-right">
								<div class="booking-card sticky-sidebar">
									<div class="booking-card-header">
										...
										<div class="price-header booking" style="display: none;">
											<h4 class="booking-title">Review Booking</h4>
											<div class="booking-price">
												<p class="text-x-large"><?php the_title(); ?></p>
											</div>
										</div>
										<?php
										$icon_id = get_post_meta(get_the_id(), 'guesty_property_icon_id', true);
									...
									<div class="date-picker-mock">
										...
										<span class="clear-dates" style="display:none;"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none">
									...
									<div class="guesty-booking-cost" style="display:none;">
										<div class="booking-details-container">
											<div class="booking-item">
												<label>Check in</label>
												<div id="booking_checkInDates"></div>
											</div>
											<div class="booking-item">
												<label>Check out</label>
												<div id="booking_checkOutDates"></div>
											</div>
											<div class="booking-item">
												<label>Nights</label>
												<div id="booking_nights"></div>
											</div>
											<div class="booking-item">
												<label>Guests</label>
									...
									<button class="default-btn booking-btn" disabled>Select dates</button>
									<button class="default-btn change-btn" style="display: none;">CHANGE</button>
									<button class="default-btn reserve-btn" style="display: none;">Reserve</button>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		const booking_button = document.querySelector('.booking-btn');
		const picker = new Litepicker({
			...
			parentEl: document.querySelector('.booking-calendar'),
			singleMode: false,
			autoApply: true,
			mobileFriendly: true,
			lockDays: [],
			...
			setup: (picker) => {
				...
				picker.on('clear:selection', () => {
					...
					document.querySelector('.clear-dates').style.display = 'none';
		...
		document.querySelector('.clear-dates').addEventListener('click', function() {
			picker.clearSelection();
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		const bookingCalendar = document.querySelector('.date-picker-mock');
		const priceHeaderBooking = document.querySelector('.price-header.booking');
		const priceHeader = document.querySelector('.price-header');
		const change_btn = document.querySelector('.change-btn');
		const booking_button = document.querySelector('.booking-btn');

		if (!change_btn) return;
		change_btn.addEventListener('click', function() {
			change_btn.style.display = 'none';
			bookingCostBox.style.display = 'none';
			bookingCalendar.style.display = 'flex';
			priceHeaderBooking.style.display = 'none';
			priceHeader.style.display = 'flex';
			booking_button.innerText = 'RESERVE';
			booking_button.onclick = null;
	...
	document.querySelector('.booking-btn').addEventListener('click', async function(e) {
		e.preventDefault();
...
<style>
	...
	button.default-btn {
		...
		&:disabled {
			background: var(--Grey-Light, #C4C4C4);
			border-color: var(--Grey-Light, #C4C4C4);
			color: var(--White, #FFF);
			cursor: not-allowed;
		}
	}
	...
	.booking-details-container {
		...
		border-bottom: 1px solid var(--Grey-Light, #C4C4C4);
		padding-bottom: 16px;
	}
	...
	@media (max-width: 767px) {
		...
		.booking-card.sticky-sidebar:not(.reserve-clicked) button.booking-btn {
			display: none;
		}
